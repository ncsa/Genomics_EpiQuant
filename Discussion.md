...
In scala, it is pretty obvious to me that we could mix in a trait that contains a case class with the info we want.
...
I think we should go with interfaces, because there is no need to have a case class layer in between the concrete class and the fields we want (unless we end up pattern matching against the properties case class, which we may)
