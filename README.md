...
To analyze the performance of the application and to improve certain parts of the code we used performance profilers. Yourkit Java Profiler was used to profile the code. The tool provides real-time resource utilization statistics. CPU usage graph showing total, kernel and garbage collector times is always available. There are various profiling modes that can be used for tracing, sampling and call counting. Tracing is very useful since it gives us information on how long a particular method ran for and number of times it was called. CPU Usage, memory utilization such Heap memory, Non-Heap memory usage can be viewed in realtime. A snapshot of the job can be obtained during runtime and can be used to analyze the resource utilization.
